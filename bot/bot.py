...
def main():
    ...
    dispatcher.add_handler(MessageHandler(Filters.regex(r'^Хочу быть в курсе событий!$'), news_button))
    dispatcher.add_handler(MessageHandler(Filters.regex(r'^Хочу поиграть в квиз!$'), quiz))

    dispatcher.add_handler(ConversationHandler(
        entry_points=[MessageHandler(Filters.regex(r'^Хочу поболтать!$'), start_anket)],
        states={
            'user_name': [MessageHandler(Filters.text & (~Filters.command), user_name)],
            'respect': [MessageHandler(Filters.regex(r'^Конечно на Ты!$|^Лучше на Вы.$'), respect)],
            'user_age': [MessageHandler(Filters.regex(r'^\d{,2}$'), user_age)],
            'user_phone':[MessageHandler(Filters.contact | Filters.regex(r'^Мы еще не так близко знакомы, чтобы обмениваться телефонами!$'), user_phone)],
            'user_bio': [MessageHandler(Filters.text & (~Filters.command | Filters.regex(r'^Пропустить$')), user_bio)],
            'user_evaluate': [MessageHandler(Filters.regex(r'^[1-5]$'), user_evaluate)],
            'user_comment': [MessageHandler(Filters.text & (~Filters.command) | Filters.regex(r'^Пропустить$'), questionnaire_completing)],
        },
        fallbacks = [MessageHandler(Filters.text | Filters.video | Filters.photo | Filters.document, dontknow)]
    ))

    dispatcher.add_handler(CallbackQueryHandler(inline_button_news, pattern=(r'subscribe|unsubscribe')))
    dispatcher.add_handler(CallbackQueryHandler(inline_button_quiz, pattern=(r'^[1-4]$')))
    dispatcher.add_handler(CallbackQueryHandler(inline_button_theme, pattern=(r'^bot/quiz_questions/[\w\s]+.txt$')))
