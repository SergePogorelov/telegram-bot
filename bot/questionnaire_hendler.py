...
def user_age(update, context):
    ...
    button2 = KeyboardButton(
        "Мы еще не так близко знакомы, чтобы обмениваться телефонами!"
    )
    keyboard = ReplyKeyboardMarkup(
        [[button1], [button2]], resize_keyboard=True, one_time_keyboard=True
    )
...
def user_phone(update, context):
    ...
    if context.user_data["respect"] == "тебе":
        text = "Расскажи"
    update.message.reply_text(
        text=f"{text}, пожалуйста пару слов о себе.",
        reply_markup=ReplyKeyboardMarkup(
...
def user_bio(update, context):
    ...
    update.message.reply_text(
        text="Оцените заполнение анкеты\n*от 1 до 5*?\n\n_5 - очень понравилось\n1- совсем плохо_",
...
def questionnaire_completing(update, context):
    ...
    if context.user_data.get("user_phone"):
        phone = (
            f'Твой номер телефона - *{context.user_data["user_phone"]}*, жди звонка ;)'
        )
