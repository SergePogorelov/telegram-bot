...
*{theme}*""", parse_mode=ParseMode.MARKDOWN)
...
def get_next_question(update: Updater, context: CallbackContext):
    ...
    if question:
        ...
        context.bot.send_message(chat_id=update.effective_chat.id, text=f'*Вопрос номер: {context.chat_data["num_q"]}*\n\n{question}\n{"".join(answers_text)}', parse_mode=ParseMode.MARKDOWN, reply_markup=get_keyboard())
    else:
        quiz_file.close()
...
def start_quiz(update: Updater, context: CallbackContext):
    context.bot.send_message(chat_id=update.effective_chat.id, text='Выберите тему: ', reply_markup=get_keyboard_theme())
